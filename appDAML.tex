...
Problema della MSE con dataset asimmetrici: assume che siano gaussiani. Soluzioni: 0-1 Loss, problematica nel calcolo del gradiente; altra solizuzione
la \textbf{Sigmoide}
\[
p(C_1|x)=\frac{1}{1+\exp^{-a}} = \sigma(a), \qquad a=\ln\frac{p(C_1|x)}{p(C_2|x)}\text{ (logit)}
\]

Regressione logistica: data la funzione $a(x)=W^Tx+w_0$ lineare nei pesi, abbiamo $\sigma(a(x))$.\newline
Per trovare i pesi ottimali, si può scrivere la funzione di massima verosimiglianza bernoulliana
\[
L(w) = \prod_{i=1}^n p(C_1|x_i)^{y_i}p(C_2|x_i)^{1-y_i}
\]
da cui, estraendo il negativo del logaritmo, si ottiene la \textbf{Binary Cross-Entropy Loss}:
\[
L(w) = - \sum_{i=1}^n [y_i\ln p(C_1|x_i) + (1-y_i)\ln p(C_2|x_i)]
\]

La regressione lineare consiste in un layer in cui si accoppiano features e pesi (lineare) il cui risultato va in pasto a una sigmoide. Prima di ciò, si può
fare \textit{feature engineering} per adattare le features a una selezione lineare.

\subsubsection*{Classification metrics}
\[
\text{Accuracy} = \frac{TP + TN}{TP+FP+TN+FN} \qquad \text{Recall}=\frac{TP}{TP + FN}
\]
\[
\text{Precision} = \frac{TP}{TP+FP} \qquad \text{Specificity}=\frac{FN}{TN+FP}
\]
\[
...
Metodo iterativo del primo ordine per trovare il minimo della Loss. Effettua un passo nel verso opposto del
gradiente calcolato rispetto ai pesi, e si muove di una distanza $\eta$ detta \textit{Learning Rate}.
\[
w^{(t-1)} = w^{(t)} - \eta \grad{}_w L(w^{(t)})
\]
Il learning rate ottimale è l'inverso dell'Hessiana.

\subsubsection*{Stochastic Gradient Descent (SGD)}
Aggiunge una componente di casualità alla direzione del GD. Se con Mini-Batch, divide causalmente il
dataset in mini batches e calcola il GD su ognuno dei singoli batches singolarmente.

\subsubsection*{SGD with Momentum}
Si può aggiungere una componente di "inerzia" al SGD, associando un momento.
Questo permette di uscire dai minimi locali avendo una memoria della direzione di spostamento nello spazio dei parametri.
\[
v_{t+1} = \rho v_t - \eta\grad{L(w_t)}\ , \qquad w_{t+1} = w_t + v_{t+1}
\]
Un metodo più vantaggioso è usare il momento di Nesterov:
\[
v_{t+1} = \rho v_t - \eta\grad{L(w_t + \rho v_t)}
\]

\subsubsection*{Adaptive Learning Rate}
Il learning rate si adatta in modo da essere sempre l'inverso dell'hessiana di L, accorciandosi quando il gradiente 
è grande e velocizzando la convergenza quando nei plateau.

\subsubsection*{ADAM (Adaptive Moment Estimation)}
Unisce i vantaggi del momento e dell'adaptive learning rate.


\section{Decision Trees}
Binary query function $q(x)$ in ogni Branch node, che classifica 0 o 1.
Non avendo un numero definito di parametri non si può fare il GD: \textbf{Greedy Agorithm}.\newline
Si sceglie lo split massimizzando il delta dell'impurity function $L(R)$:
\[
\max_{j,T}\{L(R_p)-(p_PL(R_1) + p_FL(R_2))\}
\] 
Come impurità si possono usare: Misclassification error, Entropia o Gini index. Per la regressione,
si usa il MSE.\newline
Il greedy algorithm non ha problemi di convergenza come il GD. Vediamo le stopping conditions:
\begin{itemize}
    \item[-] Minimum leaf size
    \item[-] Effective number of events (per eventi pesati)
    \item[-] Perfect separation: non si splitta più se in un nodo c'è una sola classe
    \item[-] Insufficient improvement:  stop quando gli split non migliorano la performance (not so good)
\end{itemize}

\subsection*{Regolarizzazione}
\textbf{Pruning}: espandiamo il Tree e a posteriori tagliamo dei sub-tree in base a
a una loss che penalizza il numero di foglie.\newline

\textbf{Feature Importance}: valutazione del peso delle features nella decisione, per togliere
    quelle che non contribuiscono e semplificare il modello. $Imp(f)=Q_0 - Q_f$, con $Q$ quality measure
    calcolata prima con tutte le features e poi rimuovendo una feature $f$.\newline

\textbf{Ensembling}: combinazione di più modelli per ottenere un modello migliore\newline

\textbf{Bagging (Bootstrap Aggregating)}: produce $m$ dataset a partire dal dataset iniziale,
creati mescolando sub-dataset con possibile ripetizione. RIDUCE LA VARIANZA.

\textbf{Random Forest}: Bagging + decision trees + more randomness.\newline
Si divide il dataset in tanti subset (bagging) e si costruisce un tree per ogni subset. In ogni nodo del tree vengono
usate solo una parte delle features per avere alberi indipendenti tra loro. La decisione finale è il voto
pesato di ogni tree.\newline

\textbf{Boosting}: opposto del bagging, riduce il bias di modelli a bassa varianza (weak learners).\newline
Train di modelli in sequenza, sistemando gli errori dei modelli precedenti, poi si prende la media pesata dei 
modelli come best decision.\newline
Ad es, \textbf{Adaboost} assegna i pesi con una loss esponenziale.

\textbf{Gradient Boosting}: simile al Gradient Descent, però nello spazio dei modelli. La loss ha un termine di loss comune e un termine di regolarizzazione.\newline
Segue uno schema additivo, creando un albero a cui fa fare una predizione, e poi crea un altro albero per correggere gli errori del precedente in maniera iterativa.
La predizione finale è data dalla somma delle predizioni degli alberi, mentre la funzione oggettiva al passaggio $t$ è data da 
\[
\text{obj}^{(t)} = \sum_{i=1}^n l(y_i, \hat{y}_i^{(t-1)} + f_t(x_i)) + \Omega(f_t) + const
\]
dove l'indice $i$ indica gli eventi.
...
Con \textbf{risposta di un rivelatore} ci si riferisce alla misura ottenuta da un rivelatore che ha ricevuto uno specifico input. I rivelatori non producono misure esatte
...
Produzione: acccoppiamento con fermioni (in particolare t che ha la massa, e quindi l'accoppiamento con l'Higgs, maggiore) ggH (gluon fusion con loop t 90$\%$) e ttH (2g$\rightarrow$t$\bar{\text{t}}$t$\bar{\text{t}}$); accoppiamento con bosoni vettori VBF (vector boson fusion, 2q$\rightarrow$qqWW/qqZZ), VH (q$\bar{\text{q}}\rightarrow$W/Z$\rightarrow$W/Z+H).\newline
...
Il compito della rete avversaria è di imparare $p(z|F_c(X))$, quindi indovinare il valore della variabile di disturbo $z$ a partire dell'output $F_c(X)$ del classificatore. 
Ma il classificatore verrebbe penalizzato se il suo output permettesse all'avversaria di ottenere questo risultato.
